...
class AIDisputeResolver(gl.Contract):
    ...
    def _normalize_address(self, user: str) -> Address:
        return Address(user.strip())
    ...
    def submit_dispute(self, claim: str, evidence: str) -> dict:
        ...
        try:
            ...
            candidate_verdict = str(data.get("verdict", "insufficient_evidence")).strip().lower()
            if candidate_verdict in {"guilty", "not_guilty", "insufficient_evidence"}:
    ...
    @gl.public.view
    def get_my_dispute_ids(self, user: str) -> list:
        normalized_user = self._normalize_address(user)
        ids = []
        i = 1
        upper = int(self.next_dispute_id)
        while i < upper:
            uid = u64(i)
            if uid in self.dispute_owner and self.dispute_owner[uid] == normalized_user:
                ids.append(i)
            i += 1
        return ids

    @gl.public.view
    def get_my_disputes(self, user: str) -> list:
        normalized_user = self._normalize_address(user)
        out = []
        i = 1
        upper = int(self.next_dispute_id)
    ...
    @gl.public.view
    def get_my_dispute_ids_self(self) -> list:
    ...
    @gl.public.view
    def get_my_disputes_self(self) -> list:
        user = gl.message.sender_address
        out = []
        i = 1
        upper = int(self.next_dispute_id)
